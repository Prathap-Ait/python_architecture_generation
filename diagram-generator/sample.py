...
def lambda_handler(event, context):
    """
    AWS Lambda handler to generate architecture diagram and upload to S3.
    Expects API Gateway event with body containing usecaseDetails, bucketName, folderName.
    """
    print("RAW EVENT:", json.dumps(event))
    
    try:
        # Support both local testing (direct dict) and API Gateway (body field)
        if isinstance(event, dict) and "usecaseDetails" in event:
            body = event
            print("Using event as body directly (local testing mode)")
        else:
            # Step 1: Extract raw body string from API Gateway event
            ...
            try:
                ...
                return {
                    "statusCode": 400,
                    "body": json.dumps({"error": "Invalid JSON in body", "details": str(e)})
                }
        
        print("PARSED BODY:", body)
        
        # Step 3: Extract dynamic values from parsed body
        bucket_name = body.get("bucketName", "")
        folder_name = body.get("folderName", "")
        usecase_id = body.get("usecaseDetails", {}).get("usecaseId", "")
        ...
        if not usecase_id:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "usecaseId is required"})
            }
        
        # Generate the diagram (pass entire body as it contains usecaseDetails)
        image = generate_architecture_diagram(body)
        
        # Upload to S3 with usecaseId as filename
        upload_image_to_s3(image, bucket_name, folder_name, usecase_id)
        
        return {
            "statusCode": 200,
            "body": json.dumps({
                "message": "Architecture diagram generated and uploaded successfully",
                "s3_location": f"s3://{bucket_name}/{folder_name}/{usecase_id}.png"
            })
        }
...
def ensure_list(value):
    """Ensures the value is a list. Converts string to single-item list."""
...
def generate_architecture_diagram(usecase_json):
        """
        Generate the architecture diagram based on usecase_json data.
        ...
        # ============================================================
        # Dynamic UI Tag/Chip Bar helper function
        # ============================================================
        ...
        ui_tag_items = ensure_list(usecase_json.get("usecaseDetails", {}).get("department", []))
        ...
        def draw_compact_centered_subboxes(parent_box, label, items, box_color, text_color, outline_color, font, y_offset):
            ...
            draw_smooth_rounded_box(draw, parent_box_xy, radius=10, fill='white', outline=color_gray_border, width=1)
        ...
        platforms = ensure_list(usecase_json.get("usecaseDetails", {}).get("platform", []))
        next_y = box_xy[1] + 38
        ...
        # ============================================================================
        # Cloud Services Section (Inside Agentic AI Solution)
        # ============================================================================
        ...
        try:
            font_title_large = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 15) 
        except:
        ...
        line_heights = [draw.textbbox((0, 0), line, font=font_small)[3] - draw.textbbox((0, 0), line, font=font_small)[1] for line in lines]
        text_widths = [draw.textbbox((0, 0), line, font=font_small)[2] - draw.textbbox((0, 0), line, font=font_small)[0] for line in lines]
        max_text_width = max(text_widths)
        total_text_height = sum(line_heights)

        # Center icon and text group vertically in the box
        group_height = max(icon_h, total_text_height)
        group_y = y_row2 + (box_height_row2 - group_height) // 2

        icon_x = x_box4 + (box_width - (icon_w + text_gap + max_text_width)) // 2
        icon_y = group_y + (group_height - icon_h) // 2

        text_x = icon_x + icon_w + text_gap
        text_y = group_y + (group_height - total_text_height) // 2
        ...
        text_x = icon_x + icon_w + text_gap
        text_y = y_row2 + (box_height_row2 - text_height) // 2
        
        if ai_risk_icon_img:
            image.paste(ai_risk_icon_img, (int(icon_x), int(icon_y)), ai_risk_icon_img)
        draw.text((int(text_x), int(text_y)), "AI risk register", fill=color_black, font=font_small)
        ...
        icon_x = x_box6 + (box_width - (icon_w + text_gap + max_text_width)) // 2
        icon_y = group_y + (group_height - icon_h) // 2
        ...
        llm_items = ensure_list(usecase_json.get("usecaseDetails", {}).get("baseModelName", []))
        ...
        # =========================
def upload_image_to_s3(image, bucket_name, folder_name, usecase_id):
            ...
            if not folder_name:
                ...
                return
            if not usecase_id:
                print("❌ Usecase ID not provided in JSON.")
                return
            
            # Use usecaseId as the filename
            object_name = f"{usecase_id}.png"
...
if __name__ == "__main__":
    lambda_handler(
        event={
            "usecaseDetails": {
                "usecaseId": "AI-UC-AST-001",
                "department": ["Claims Processing" , "Customer Service" , "IT Support"],
                "platform": "GCP , Azure",
